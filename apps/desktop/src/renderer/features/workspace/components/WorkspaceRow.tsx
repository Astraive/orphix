import type { Workspace } from "../stores/canvas-store";
...
export function WorkspaceRow({ workspace, wsIdx, isActiveWorkspace }: WorkspaceRowProps) {
  ...
  return (
    <div
      ...
      style={{
        transition: `opacity 500ms ease, transform ${TRANSITION_CANVAS}`,
    ...
    >
      {winCount === 0 ? (
        <div className="w-full text-center select-none">
          {isActiveWorkspace ? (
            <p className="text-sm tracking-[0.4em] uppercase animate-pulse font-mono" style={{ color: "var(--orphix-color-primary)", opacity: 0.5 }}>
              Alt+N to open window
            </p>
          ) : (
            <p className="text-sm tracking-[0.4em] uppercase font-mono" style={{ color: "var(--orphix-color-text-subtle)", opacity: 0.2 }}>
              empty workspace
            </p>
          )}
        </div>
              ...
              >
                <div
                  ...
                  style={{
                    borderRadius: "12px",
                    border: isActiveWin
                      ? "1.5px solid color-mix(in srgb, var(--orphix-color-primary) 50%, transparent)"
                      : "1.5px solid color-mix(in srgb, var(--orphix-color-primary) 6%, transparent)",
                    background: "color-mix(in srgb, var(--orphix-color-base-background) 60%, transparent)",
                    overflow: "hidden",
                    boxShadow: isActiveWin ? "0 0 24px color-mix(in srgb, var(--orphix-color-primary) 5%, transparent)" : "none",
                  }}
                >
                  <PaneRenderer
                    layout={win.layout}
