...
import type { FileEntry } from "@shared/types/common";
...
export const useFileStore = create<FileStore>()((set, get) => ({
  ...
  setRootPath: async (path: string) => {
    set({ rootPath: path, tree: [], expanded: {}, selected: path });
    try {
  ...
  select: (path) => set({ selected: path, selectedSet: path ? new Set([path]) : new Set() }),
  selectMulti: (path) => set((s) => {
    const next = new Set(s.selectedSet);
    if (next.has(path)) next.delete(path); else next.add(path);
    return { selectedSet: next, selected: path };
  }),
  selectRange: (paths) => set((s) => {
    const next = new Set(s.selectedSet);
    for (const p of paths) next.add(p);
    return { selectedSet: next };
  }),
  clearSelection: () => set({ selectedSet: new Set(), contextMenu: null }),
